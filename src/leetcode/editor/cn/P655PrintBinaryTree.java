...
class Solution {
    public List<List<String>> printTree(TreeNode root) {
        ...
        printTree(root, lists, 0, 0, width);
        return lists;
    }

    private void printTree(TreeNode root, List<List<String>> lists, int y, int l, int r) {
        if (root == null) {
            return;
        }
        lists.get(y).set((l + r) / 2, root.val + "");

        printTree(root.left, lists, y + 1, l, (l + r) / 2);
        printTree(root.right, lists, y + 1, (l + r + 1) / 2, r);
    }
